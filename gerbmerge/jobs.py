...
KHK: Fixed a problem with metric EXCELLON files from KiCAD
KHK: modified layout.cfg, config.sys, gerbmerge.py and jobs.py
...
#   D03 -- flash aperture
#KHK   G05 -- drill mode
#KHK   G04 -- comment in Gerber, G04 dwell time in RS 274X
#KHK   G90/G91 -- Incremental vs. Absolute Coordinates
#KHK   G70/G71 -- Inches vs. Millimeters
...
class Job:
  ...
  def width_in(self):
    ...
    if config.Config['measurementunits'] == 'inch':
      ...
      return float(round((self.maxx-self.minx)*0.001,2)) # KHK suppress rounding errors
  ...
  def height_in(self):
    ...
    if config.Config['measurementunits'] == 'inch':
      ...
      return float(round((self.maxy-self.miny)*0.001,2)) # KHK suppress rounding errors
...
# end basic diptrace fixes
	  ...
	  # KHK added 05 - drill mode
          if gcode in [05 ,54, 70, 90, 71]:
            continue
              ...
              if newRect:
                ...
                if min(newRectWidth, newRectHeight) >= 10: # sdd - change for metric case at some point
                  ...
                  newAP = aptable.Aperture(aptable.Rectangle, 'D??', \
                            util.gerb2in(newRectWidth), util.gerb2in(newRectHeight))   # KHK todo: do I need adjustment for metric Kicad 3.3 drillfiles here, too? Did not think about this topic at this moment.
